...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  console.log('[LLM] Has response_format:', !!payload.response_format);
  ...
  console.log('[LLM] Request successful, status:', response.status);
  
  const result = await response.json();
  console.log('[LLM] Result has choices:', !!result?.choices);
  console.log('[LLM] Choices count:', result?.choices?.length || 0);
